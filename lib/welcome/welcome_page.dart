...
import '../models/get_user_info.dart';
...
class _WelcomePageState extends State<WelcomePage> {
  ...
  Future<void> _initData() async {
     CallApi().getPublicData("werkbonnen").then((response){
       ...
       setState(() {
        ...
        articles = list.map((model)=>Werkbon.fromJson(model)).toList();
       });
     });
     // CallApi().getCurrentUser("user").then((response){
     //   setState(() {
     //     List list = (jsonDecode(response.body)['data'] as List<dynamic>) ;
     //     // Iterable list = json.decode(response.body)['data'];
     //     currentUser = list.map((model)=>UserData.fromJson(model));
     //   });
     // });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        articles[i].createdAt==null?"Hallo ": articles[i].createdAt,
